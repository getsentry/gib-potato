<template>
  ...
  <div class="mt-2 mb-32">
    ...
    <div class="py-4">
      ...
      <p class="mt-1 text-sm text-zinc-500">
        You have <strong>{{ user.potato_left_today }}</strong> 🥔 left to gib today. Your potato do
        reset in <strong>{{ user.potato_reset_in_hours }}</strong> hours and
        <strong>{{ user.potato_reset_in_minutes }}</strong> minutes.
      </p>
      <p class="mt-1 text-sm text-zinc-500">
        You did gib <strong>{{ user.sent_count ?? 0 }}</strong> 🥔 and did receive
        <strong>{{ user.received_count ?? 0 }}</strong> 🥔 since you started potatoing
        <strong
          >{{
      ...
      >
        ...
        <span
          class="bg-purple-100 text-purple-800 text-sm font-medium ml-auto px-2.5 py-0.5 rounded dark:bg-purple-900 dark:text-purple-300"
        >
        ...
        >
          <div class="flex space-x-3">
            <template v-if="message.sender_user_id === user.id">
              ...
              <div class="flex-1 space-y-1">
                <div class="flex items-center justify-between">
                  <h3 class="text-sm font-medium">To {{ message.received_user.slack_name }}</h3>
                ...
                <p class="text-sm text-zinc-500">
                  You did gib <strong>{{ message.amount }}</strong> 🥔 to
                  {{ message.received_user.slack_name }}
                </p>
              </div>
            </template>
            <template v-if="message.receiver_user_id === user.id">
              ...
              <div class="flex-1 space-y-1">
                <div class="flex items-center justify-between">
                  <h3 class="text-sm font-medium">From {{ message.sent_user.slack_name }}</h3>
                  <p class="text-sm text-gray-500">
                    {{
                      new Date(message.created).toLocaleDateString('en-us', {
                        year: 'numeric',
                ...
                <p class="text-sm text-zinc-500">
                  You did receive <strong>{{ message.amount }}</strong> 🥔 from
                  {{ message.sent_user.slack_name }}
                </p>
              </div>
            </template>
          </div>
